...
export default function OrderElement({ onClick, order, from }: IOrderElementProps) {
    ...
    .main} onClick={onClick}>
            ...
            .content}>
                ...
                {from === 'profile' &&
                    <p className={`${style
                    [status.color]} text text_type_main-default mb-4`}>{status.value}</p>
                }
                            ...
                            ? ingredientsInOrder.slice(0, 6).map((ing, index) => {
                                ...
                                .ing} key={index}>
                                        {ingredientsInOrder.length > 6 && (
                                            <span className={`text text_type_digits-default ${style
                                            .iconNumber}`}>+{ingredientsInOrder.length - 6}</span>
                                        )}
                        
                                        <img src={ing.image_mobile} alt="" className={`${style
                                        .ingIcons}`} />
                                    </span>
                                )})
                                :
